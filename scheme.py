...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        scm_pro = scheme_eval(first, env)
        args = rest.map(lambda x: scheme_eval(x, env))
        result = scheme_apply(scm_pro, args, env)
    return result
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('number of argument values does not match formal parameters')
        else:
            while len(formals) > 0:
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        para_frame = self.env.make_child_frame(self.formals, args)
        return para_frame
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    exp_first = expressions.first
    if scheme_symbolp(exp_first):
        check_form(expressions, 2, 2)
        # BEGIN PROBLEM 6
        env.define(exp_first, scheme_eval(expressions.second.first, env))
        return exp_first
        # END PROBLEM 6
    elif isinstance(exp_first, Pair) and scheme_symbolp(exp_first.first):
        # BEGIN PROBLEM 10
        ex_sec = expressions.second
        tar_sec = exp_first.second
        env.define(exp_first.first, LambdaProcedure(tar_sec, ex_sec, env))
        return exp_first.first
        # END PROBLEM 10
    else:
        wrong = exp_first.first if isinstance(exp_first, Pair) else exp_first
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if len(claus) == 0:
                ...
                return eval_all(claus, env)
            return scheme_eval(claus.first,env, True)
            # END PROBLEM 14
        expressions = expressions.second
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(exp_first):
        ...
        ex_sec = expressions.second
        tar_sec = exp_first.second
        env.define(exp_first.first, MacroProcedure(tar_sec, ex_sec, env))
        return exp_first.first
    else:
        wrong = exp_first.first if isinstance(exp_first, Pair) else exp_first
        raise SchemeError('non-symbol: {0}'.format(wrong))
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            args = rest.map(lambda o: scheme_optimized_eval(o, env))
